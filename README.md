...
This branch contains the code for the Experiment 2 that were doing. This code gives you the critical nodes when no seed set context has been given. Note that this code also contains the validationCheck() method. There is another branch called <i>CodeContaining_topCrit_WithoutAnySeedSetContextGiven_ForPosterPresentation</i> which does not contain this validationCheck() method and instead directly gives you the critical nodes in the input graph.
